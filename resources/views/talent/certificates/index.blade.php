    ...
    <div class="space-y-6" x-data="{ 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($certificates as $cert)
                <div
                    class="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-xl transition-all group relative">
                    <!-- Dropdown Menu -->
                    <div class="absolute top-4 right-4 z-10" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            <a href="{{ asset('storage/' . $cert->file_path) }}" target="_blank"
                                class="flex items-center gap-3 px-4 py-3 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                    ...
                    <div
                        class="h-52 bg-slate-100 dark:bg-slate-900 flex items-center justify-center relative group-hover:scale-105 transition-transform duration-500 overflow-hidden">
                        @php
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="font-bold text-slate-900 dark:text-white line-clamp-1 text-lg">
                                {{ $cert->nama_sertifikat }}</h3>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700">
                            <span class="px-3 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider
                                        @if(strtolower($cert->status) == 'verified' || strtolower($cert->status) == 'terverifikasi') bg-emerald-100 text-emerald-700 
